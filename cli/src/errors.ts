...
const HINTS: ReadonlyArray<readonly [RegExp, string]> = [
  ...
  [/unknown field|deny_unknown_fields|missing field|Invalid action request/i, "Wire-shape mismatch between @terminal3/t3n-sdk 5.10.0 and this node — testnet may lag the SDK. See cli/DISCREPANCIES.md and report the request_id in the developer Telegram."],
  [/trust manifest .* is malformed/i, "The node served a trust manifest that lacks a field SDK 5.10.0 requires (testnet, 2026-09-04: no rtmr1_allowlist) — `kyb doctor` lists the missing fields. To proceed on testnet set T3N_TRUST=unsafe (skips attestation; refused on production), or pin an older SDK. See cli/DISCREPANCIES.md #1."],
  ...
  [/attestation verification|rtmr|trust anchor|tdx quote/i, "Node attestation verification failed: the TDX quote does not match the pinned peer IDs / RTMR allowlist. Testnet and SDK 5.10.0 may disagree — see cli/DISCREPANCIES.md."],
